...
import {
  APP_RECEIPT_FIELDS_MAP,
  IN_APP_PURCHASE_RECEIPT_FIELDS_MAP,
  RECEIPT_FIELDS_MAP,
  ReceiptFieldsKeyNames,
  ReceiptFieldsKeyValues,
} from "./mappings";
...
export type ParsedReceipt = Partial<Record<ReceiptFieldsKeyNames, string>> & {
  ...
  IN_APP_RECEIPTS: Record<string, string>[];
...
function isReceiptFieldKey(value: unknown): value is ReceiptFieldsKeyValues {
  return Boolean(
    typeof value === "number" &&
      RECEIPT_FIELDS_MAP.has(value as ReceiptFieldsKeyValues)
  );
}

function isAppReceiptFieldKey(value: unknown): value is ReceiptFieldsKeyValues {
  return Boolean(
    typeof value === "number" &&
      APP_RECEIPT_FIELDS_MAP.has(value as ReceiptFieldsKeyValues)
  );
}

function isInAppPurchaseReceiptFieldKey(
  value: unknown
): value is ReceiptFieldsKeyValues {
  return Boolean(
    typeof value === "number" &&
      IN_APP_PURCHASE_RECEIPT_FIELDS_MAP.has(value as ReceiptFieldsKeyValues)
  );
}

function isParsedReceiptContentComplete(
  data: ParsedReceipt
): data is ParsedReceipt {
...
function extractFieldValue(field: ASN1.OctetString): string {
  ...
  if (
    fieldValue instanceof ASN1.IA5String ||
    fieldValue instanceof ASN1.Utf8String
  ) {
...
function appendField(
  ...
  inAppReceipt: Record<string, string> | null,
  name: ReceiptFieldsKeyNames,
  value: string,
  fieldKey: ReceiptFieldsKeyValues
) {
  ...
  if (isInAppPurchaseReceiptFieldKey(fieldKey) && inAppReceipt !== null) {
    inAppReceipt[name] = value;
  }
  if (isAppReceiptFieldKey(fieldKey)) {
    parsed[name] = value;
  }
}
...
function processField(
  ...
  inAppReceipt: Record<string, string> | null,
  fieldKey: number,
  fieldValue: ASN1.OctetString
) {
  if (fieldKey === IN_APP) {
    let inAppReceipt = {};
    parseOctetStringContent(parsed, inAppReceipt, fieldValue);
    parsed.IN_APP_RECEIPTS.push(inAppReceipt);
    return;
  }
...
function parseOctetStringContent(
  parsed: ParsedReceipt,
  inAppReceipt: Record<string, string> | null,
  content: ASN1.OctetString
) {
  const [contentSet] = content.valueBlock.value as ASN1.Set[];
  const contentSetSequences = contentSet.valueBlock.value.filter(
    (v) => v instanceof ASN1.Sequence
  ) as ASN1.Sequence[];
  ...
  for (const sequence of contentSetSequences) {
    ...
    if (verifiedSequence) {
      ...
      const fieldKey = (verifiedSequence.result[FIELD_TYPE_ID] as ASN1.Integer)
        .valueBlock.valueDec;
      const fieldValueOctetString = verifiedSequence.result[
        FIELD_VALUE_ID
      ] as ASN1.OctetString;

      processField(parsed, inAppReceipt, fieldKey, fieldValueOctetString);
    }
  }
}

function postprocessParsedReceipt(parsed: ParsedReceipt) {
  // parsed.IN_APP_ORIGINAL_TRANSACTION_IDS = uniqueArrayValues(
  //   parsed.IN_APP_ORIGINAL_TRANSACTION_IDS
  // );
  // parsed.IN_APP_TRANSACTION_IDS = uniqueArrayValues(
  //   parsed.IN_APP_TRANSACTION_IDS
  // );
}
...
export function parseReceipt(receipt: string): ParsedReceipt {
  ...
  parseOctetStringContent(parsed, null, content);
